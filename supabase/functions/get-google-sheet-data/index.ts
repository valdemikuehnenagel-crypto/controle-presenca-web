  ...
  try {
    ...
    if (mainRows.length >= 2) {
      ...
      const colIndex = {
        ...
        META: findCol(header, ["META", "Meta"]), // Ainda lemos, mas não usamos
      ...
      dataRows.forEach((row) => {
        ...
        processedData[kpi] ??= {};
        processedData[kpi][yearWeekKey] ??= {};
        kpiCodigoMap[kpi] ??= new Set();
        kpiCodigoMap[kpi].add(codigo);

        // *** MUDANÇA 1 AQUI ***
        // Enviamos apenas o resultado
        processedData[kpi][yearWeekKey][codigo] = {
          resultado: resultadoNum,
          resultadoRaw: resultadoStr || "-",
          // meta: metaNum, // Removido
          // metaRaw: metaStr || "-", // Removido
        };
    ...
    if (acuracidadeRows.length >= 2) {
      ...
      const colIndexAcurac = {
        ...
        NumeroWeek: findCol(header, ["NumeroWeek", "Numero_Week", "Week", "NroWeek"]),
        CODIGO: findCol(header, ["CODIGO", "Código", "SERVICE", "Svc"]),
        RESULTADO: findCol(header, ["Com Poc", "Com poc"]), // C/ POC
        META: findCol(header, ["Sem poc", "S/POC", "Sem Poc"]), // S/ POC
        GERENCIA: findCol(header, ["Gerencia", "Gerência", "Gerente"]),
        MACRO: findCol(header, ["MACRO_REGIONAL", "Macro_Regional", "Macro"]),
      };
      ...
      dataRows.forEach((row) => {
        const kpi = KPI_ACURACIDADE_NAME; // "Acuracidade de Inventário"
        ...
        const comPocStr = String(row[colIndexAcurac.RESULTADO] || "").trim(); // Era resultadoStr
        const semPocStr = String(row[colIndexAcurac.META] || "").trim(); // Era metaStr

        const comPocNum = toNum(comPocStr); // Era resultadoNum
        const semPocNum = toNum(semPocStr); // Era metaNum
        ...
        // *** MUDANÇA 2 AQUI ***
        // Enviamos com os nomes corretos
        processedData[kpi][yearWeekKey][codigo] = {
          com_poc: comPocNum,
          sem_poc: semPocNum,
          com_pocRaw: comPocStr || "-",
          sem_pocRaw: semPocStr || "-",
        };
