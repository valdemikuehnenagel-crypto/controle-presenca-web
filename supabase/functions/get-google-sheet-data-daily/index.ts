...
serve(async (req) => {
  ...
  try {
    // ---- NOVO: lê período opcional do body ----
    let body: any = {};
    try { body = await req.json(); } catch { body = {}; }
    const periodStart: string = typeof body?.periodStart === 'string' ? body.periodStart : "";
    const periodEnd: string   = typeof body?.periodEnd   === 'string' ? body.periodEnd   : "";

    // Função de inclusão: se veio período, usa intervalo; senão, mês atual
    const includeRow = (dateKey: string, monthKey: string) => {
      if (periodStart && periodEnd) return (dateKey >= periodStart && dateKey <= periodEnd);
      return (monthKey === CURRENT_MONTH_KEY);
    };
    ...
    const acuracidadeEndpoint = `https://sheets.googleapis.com/v4/spreadsheets/${SPREADSHEET_ID}/values/${encodeURIComponent(ACURACIDADE_RANGE)}`;

    const [mainResponse, acuracidadeResponse] = await Promise.all([
      fetch(mainEndpoint, { headers: { Authorization: `Bearer ${accessToken}` } }),
      fetch(acuracidadeEndpoint, { headers: { Authorization: `Bearer ${accessToken}` } }),
    ]);

    if (!mainResponse.ok) {
      throw new Error(`Erro API Sheets (Main) (${mainResponse.status}): ${await mainResponse.text()}`);
    }
    if (!acuracidadeResponse.ok) {
      throw new Error(`Erro API Sheets (Acuracidade) (${acuracidadeResponse.status}): ${await acuracidadeResponse.text()}`);
    }
    ...
    if (mainRows.length >= 2) {
      ...
      if (colIndex.DATA === -1 || colIndex.KPI === -1 || colIndex.CODIGO === -1) {
        console.error("Cabeçalho (Main) lido:", header);
        throw new Error("Colunas essenciais (DATA, KPI, CODIGO) não encontradas na aba Main.");
      }

      dataRows.forEach((row) => {
        const kpi = row[colIndex.KPI];

        // pula acuracidade aqui (tratado na outra aba)
        if (normalize(kpi) === KPI_ACURACIDADE_NORMALIZED) return;
        ...
        // ---- AQUI: filtro por período/mês ----
        if (!includeRow(dateKey, monthKey)) return;
        ...
        if (monthKey) { monthsSet.add(monthKey); dayToMonth[dateKey] = monthKey; }
        uniqueKpis.add(kpi);
        ...
        if (gerente) { if (!codigoToGerente[codigo]) codigoToGerente[codigo] = gerente; gerentesSet.add(gerente); }
        if (macro)   { if (!codigoToMacro[codigo])   codigoToMacro[codigo]   = macro;   macrosSet.add(macro);   }
      });
    }
    ...
    if (acuracidadeRows.length >= 2) {
      ...
      if (colIndexAcurac.DATA === -1 || colIndexAcurac.CODIGO === -1 ||
          colIndexAcurac.RESULTADO === -1 || colIndexAcurac.META === -1) {
        console.error("Cabeçalho (Acuracidade) lido:", header);
        throw new Error("Colunas essenciais (DIA, CODIGO, Com Poc, Sem poc) não encontradas na aba Acuracidade.");
      }
