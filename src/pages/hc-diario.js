...
let _building = false;
let _buildToken = 0;
let _needsRebuild = false;let _colabs = [];const _filters = {matriz: '', svc: '', regiao: '', gerencia: '', inicioISO: '', fimISO: ''};
const FALLBACK_HTML = `
<div class="hcd-root">
  <div class="hcd-bar">
...
</div>`;const pad2 = n => (n < 10 ? `0${n}` : `${n}`);
const toISODate = d => `${d.getFullYear()}-${pad2(d.getMonth() + 1)}-${pad2(d.getDate())}`;
const firstDayOfMonth = d => new Date(d.getFullYear(), d.getMonth(), 1);const norm = v => String(v ?? '').trim().toUpperCase();
const normalizeWeekdayPT = s =>
    norm(s).normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/Ç/g, 'C');function parseAnyToISO(v) {
    if (!v) return '';
    if (typeof v === 'string') {
        const s = v.trim();
        if (s === '0000-00-00') return '';        if (/^\d{4}-\d{2}-\d{2}/.test(s)) return s.slice(0, 10);
...
const onlyActiveConf = arr => (arr || []).filter(c => norm(c.Cargo) === 'CONFERENTE' && norm(c.Ativo || 'SIM') === 'SIM');function dsrSetFrom(c) {
    ...
    if (!_colabs.length) {        await fetchColabs(matrizesMap);
    }    const nomesPermitidos = _colabs.map(c => c.Nome);
    if (!nomesPermitidos.length) return [];    const {data, error} = await supabase.rpc('get_ferias_no_intervalo', {
    ...
    let query = supabase
        ...
        .order('Data de Desligamento', {ascending: true});    if (matrizesPermitidas !== null) query = query.in('MATRIZ', matrizesPermitidas);    if (_filters.matriz) query = query.eq('MATRIZ', _filters.matriz);
    if (_filters.svc) query = query.eq('SVC', _filters.svc);    const {data, error} = await query;
    if (error) throw error;    const desligData = Array.isArray(data) ? data : [];    const enrichedDesligados = desligData.map(d => {
    ...
    const rows = {};
    ROWS_ORDER.forEach(k => rows[k] = {});    const auxTurno = byTurnFilterAux(turno);
    const confTurno = byTurnFilterConf(turno);    const dsrCache = new Map();
    const getDSRSet = (c) => {
        ...
        const want = weekdayKey(d);        const auxElegiveis = auxTurno.filter(c => !c['Data de admissão'] || parseAnyToISO(c['Data de admissão']) <= d);
        const confElegiveis = confTurno.filter(c => !c['Data de admissão'] || parseAnyToISO(c['Data de admissão']) <= d);        const setAux = new Set(auxElegiveis.map(c => c.Nome));
        const setConf = new Set(confElegiveis.map(c => c.Nome));        const dsrAux = auxElegiveis.reduce((acc, c) => acc + (getDSRSet(c).has(want) ? 1 : 0), 0);
        const dsrConf = confElegiveis.reduce((acc, c) => acc + (getDSRSet(c).has(want) ? 1 : 0), 0);        const nomesFerias = feriasPorDia.get(d) || new Set();
        let feriasAux = 0, feriasConf = 0;
        for (const c of auxElegiveis) if (nomesFerias.has(c.Nome)) feriasAux++;
        for (const c of confElegiveis) if (nomesFerias.has(c.Nome)) feriasConf++;        let presAux = 0, presConf = 0, fal = 0, ate = 0, fe = 0, fer = 0;
        const marks = marksByDate.get(d) || [];        for (const m of marks) {
            const nome = m.Nome;
            const inAux = setAux.has(nome);
            const inConf = setConf.has(nome);
            if (!inAux && !inConf) continue;            const tipo = canonicalMark(m);            const bump = (t, grp) => {
                switch (t) {
                    case 'PRESENCA':
        ...
        const deslig = (desligRows || []).reduce((acc, r) => {
            ...
            if (iso !== d) return acc;            const rTurno = norm(r.Escala || '');
            const thisTurno = norm(turno);            if (rTurno && rTurno !== thisTurno) return acc;
            if (!rTurno && !(setAux.has(r.Nome) || setConf.has(r.Nome))) return acc;            return acc + 1;
        }, 0);        rows['LOG I'][d] = presAux;
        ...
        rows['FERIADO'][d] = fer;
        rows['ADMISSÃO'][d] = adm;
        rows['DESLIGAMENTOS'][d] = deslig;        rows['TOTAL QUADRO'][d] = (presAux + presConf) + (dsrAux + dsrConf) + (feriasAux + feriasConf) + fal + ate + fe + fer;
    }    return rows;
    ...
    _building = true;    const host = document.querySelector(HOST_SEL);
    if (!host) {
        _building = false;
        return;
    }    try {
        ...
        const prevGlobalR = _filters.regiao, prevGlobalG = _filters.gerencia;        if (window.__HC_GLOBAL_FILTERS) {
        ...
        const endEl = host.querySelector('#hcd-end');        let inicioISO = startEl?.value || _filters.inicioISO;
        let fimISO = endEl?.value || _filters.fimISO;        [inicioISO, fimISO] = clampEndToToday(inicioISO, fimISO);
        if (startEl && startEl.value !== inicioISO) startEl.value = inicioISO;
        if (endEl && endEl.value !== fimISO) endEl.value = fimISO;        const selM = document.querySelector('#hc-filter-matriz');
        const selS = document.querySelector('#hc-filter-svc');
        const selR = document.querySelector('#hc-filter-regiao');
        const selG = document.querySelector('#hc-filter-gerencia');         const prevM = _filters.matriz, prevS = _filters.svc;
        const prevR = _filters.regiao, prevG = _filters.gerencia;         if (selM) _filters.matriz = selM.value || '';
        if (selS) _filters.svc = selS.value || '';
        if (selR) _filters.regiao = selR.value || '';
        if (selG) _filters.gerencia = selG.value || '';         if (!inicioISO || !fimISO) {
            _building = false;
            return;
        }        const dates = eachDateISO(inicioISO, fimISO);        const t1El = host.querySelector('#hcd-t1');
        ...
        if (gEl) emptyTable(gEl, 'QUADRO GERAL');        const filtrosMudaram =
            ...
            prevGlobalG !== _filters.gerencia;         const matrizesMap = await loadMatrizesMapping();
        if (myToken !== _buildToken) return;        if (!_colabs.length || filtrosMudaram) {
            try {
        ...
        if (myToken !== _buildToken) return;        const marksByDate = new Map();
        ...
        const g = sumRowsByDate(sumRowsByDate(r1, r2, dates), r3, dates);        if (t1El) t1El.innerHTML = tableFromRows('TURNO 1', dates, r1);
    ...
    if (!host) return;    let startEl = host.querySelector('#hcd-start');
    let endEl = host.querySelector('#hcd-end');    if (!startEl || !endEl) {
        const ensureBar = () => {
        ...
        const bar = ensureBar();        if (!startEl) {
    ...
    const eDef = toISODate(now);    if (!parseAnyToISO(startEl.value)) startEl.value = sDef;
    if (!parseAnyToISO(endEl.value)) endEl.value = eDef;    _filters.inicioISO = startEl.value.slice(0, 10);
    _filters.fimISO = endEl.value.slice(0, 10);    startEl.style.display = 'none';
    endEl.style.display = 'none';    let bar = host.querySelector('.hcd-bar .hcd-filters');
    if (!bar) {
        const hcdBar = document.createElement('div');
        hcdBar.className = 'hcd-bar';
        const hcdFilters = document.createElement('div');
    ...
    if (!btn) {
        ...
        const curEnd = _filters.fimISO || endEl.value || eDef;        const today = new Date();
      ...
      <div class="cdp-card">
        ...
        }        document.body.appendChild(overlay);        const elStart = overlay.querySelector('#hcd-period-start');
        ...
        overlay.querySelector('#cdp-prevmo').onclick = () => {
            ...
            if (eVal > todayISO) eVal = todayISO;            _filters.inicioISO = sVal;
            _filters.fimISO = eVal;
            startEl.value = sVal;
            endEl.value = eVal;            buildHCDiario();
            close();
        };
    };    startEl.onchange = () => {
    ...
    if (!host) return;    if (!_mounted) {
        ...
        host.innerHTML = html;
        _mounted = true;        host.querySelector('#hcd-refresh')?.remove();        try {            const matrizesMap = await loadMatrizesMapping();
            await fetchColabs(matrizesMap);
        } catch {
            _colabs = [];
        }
    ...
    const f = ev?.detail || {};    _filters.matriz = f.matriz || '';
    _filters.svc = f.svc || '';
    _filters.regiao = f.regiao || '';
    _filters.gerencia = f.gerencia || '';     if (_building) {
        _needsRebuild = true;
        return;
    }    if (!_mounted) {
        await ensureHCDiarioMountedOnce();
    } else {
        try {            const matrizesMap = await loadMatrizesMapping();
            await fetchColabs(matrizesMap);
        } catch {
            _colabs = [];
        }
