...
function aplicarFiltrosRh() {
    ...
    const filtrados = rhState.dadosBrutos.filter(item => {
        ...
        const dataConsiderada = item.DataAdmissaoReal || item.DataInicioDesejado || '1900-01-01';
        const isFuture = dataConsiderada > hoje;
...
function renderTable(dataToRender) {
    ...
    dataToRender.forEach((colaborador) => {
        ...
        else if (colaborador.StatusDesligamento === 'RECUSADO') tr.classList.add('row-rejected');
        const nomeCelula = formatarNomeColaborador(colaborador);
        const admissaoOriginal = formatDateLocal(colaborador['Data de admissão']);
        const absTotal = (colaborador['Total Faltas'] || 0) + (colaborador['Total Atestados'] || 0);
        tr.innerHTML = `
...
function updatePendingImportCounter() {
    ...
    pendentesFiltrados.forEach(vaga => {
        const nome = (vaga.CandidatoAprovado || '').toUpperCase().trim();
        const dataInicio = vaga.DataAdmissaoReal || vaga.DataInicioDesejado || '1900-01-01';
    ...
    const total = countParaFechar + countAtuais + countFuturas;
    if (total > 0) {
        showPendingImportAlert(countParaFechar, countAtuais, countFuturas);
    } else {
        hidePendingImportAlert();
    }
}
...
async function checkPendingImports() {
    ...
    let query = supabase
        .from('Vagas')
        .select('CandidatoAprovado, MATRIZ, Gestor, Cargo, EmpresaContratante, DataInicioDesejado, DataAdmissaoReal')
        .eq('Status', 'EM ADMISSÃO')
        .or('Cargo.ilike.%CONFERENTE%,Cargo.ilike.%AUXILIAR DE OPERAÇÕES%');
    if (matrizesPermitidas !== null) {
...
function showPendingImportAlert(qtdFechar, qtdAtuais, qtdFuturas) {
    ...
    if (!alertDiv) {
        ...
        alertDiv.style.cssText = `
            ...
            white-space: nowrap; 
            overflow: hidden;
            font-size: 14px; /* Tamanho padrão de leitura */
            color: #374151;
        `;
    ...
    const styleBadgeBase = "padding: 2px 8px; border-radius: 4px; font-weight: 600; font-size: 12px; display: inline-flex; align-items: center; gap: 4px; line-height: 1.4;";
    const styleFechar = qtdFechar > 0
        ? "background: #dcfce7; color: #166534; border: 1px solid #86efac;"
        : "background: #f3f4f6; color: #9ca3af; border: 1px solid #e5e7eb; opacity: 0.6;";
    const styleAtuais = qtdAtuais > 0
        ? "background: #eff6ff; color: #2563eb; border: 1px solid #bfdbfe;"
        : "background: #f3f4f6; color: #9ca3af; border: 1px solid #e5e7eb; opacity: 0.6;";
    const styleFuturas = qtdFuturas > 0
        ? "background: #fff7ed; color: #c2410c; border: 1px solid #fed7aa;"
        : "background: #f3f4f6; color: #9ca3af; border: 1px solid #e5e7eb; opacity: 0.6;";
    const total = qtdFechar + qtdAtuais + qtdFuturas;
    const plural = total > 1 ? 'novas vagas' : 'nova vaga';
    alertDiv.innerHTML = `
        <div style="flex-shrink: 0; display: flex; align-items: center;">
            <img src="./imagens/atencao.gif" alt="Atenção" style="width: 28px; height: 28px; object-fit: contain;">
        </div>        <div style="font-weight: 500;">
            Você tem <strong>${total} ${plural} pendentes</strong>:
        </div>        <div style="display: flex; align-items: center; gap: 8px;">
            <span style="${styleBadgeBase} ${styleFechar}" title="Colaborador já ativo">
                ✅ P/ Fechar: ${qtdFechar}
            </span>
            <span style="${styleBadgeBase} ${styleAtuais}" title="Admissão hoje ou passada">
                🔥 Atuais: ${qtdAtuais}
            </span>
            <span style="${styleBadgeBase} ${styleFuturas}" title="Admissão futura">
                🚀 Futuras: ${qtdFuturas}
            </span>
        </div>        <div style="height: 24px; width: 1px; background: #e5e7eb; margin: 0 4px;"></div>        <div style="display: flex; align-items: center; gap: 4px; font-size: 13px;">
            <span style="color: #6b7280; font-weight: 600; text-transform: uppercase; font-size: 11px;">AÇÃO:</span>
            <span>Clique em</span>
            <strong style="color: #003369; cursor: pointer; text-decoration: underline;" onclick="const btn = document.getElementById('add-colaborador-btn'); if(btn) btn.click();">Adicionar</strong>
            <span>></span>
            <strong>Importar RH</strong>
        </div>
    `;
...
function handleSort(column) {
    ...
    if (currentIcon) currentIcon.textContent = currentSort.direction === 'desc' ? '⬇️' : '⬆️';
...
async function fetchColaboradores() {
    ...
    try {
        ...
        if (cached) {
            ...
            if (isSameUser && isValidTime) {
                ...
                cachedColaboradores = data.map(c => {
    ...
    try {
        ...
        let data = await fetchAllWithPagination(query);
...
export async function init() {
    ...
    setupObserver();
    const thPresenca = document.getElementById('th-presenca');
    const thFalta = document.getElementById('th-falta');
    const thAtestado = document.getElementById('th-atestado');
    const thAbsTotal = document.getElementById('th-abstotal');
    if (thPresenca) thPresenca.onclick = () => handleSort('Total Presença');
    if (thFalta) thFalta.onclick = () => handleSort('Total Faltas');
    if (thAtestado) thAtestado.onclick = () => handleSort('Total Atestados');
    if (thAbsTotal) thAbsTotal.onclick = () => handleSort('AbsTotal');
