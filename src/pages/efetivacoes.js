...
let _filtersPopulated = false;function populateFilters(allColabs, matrizesMap) {
    ...
    if (isKN) {
        ...
        submitBtn.textContent = 'Confirmar Desligamento';
        submitBtn.style.backgroundColor = '#4338ca';
    } else {
        if (emailContainer) emailContainer.style.display = '';
        if (bodyContainer) bodyContainer.style.display = '';
...
let inputCc;function initControleVagas() {
    ...
    reader.onload = (e) => {
        try {
            ...
            const row = json[0];
            const dadosMapeados = mapearDadosImportacao(row);
            openVagasModal(dadosMapeados, true);
            document.getElementById('modal-title').textContent = "Nova Vaga (Importada)";
            window.customAlert(`Dados importados!\nCandidato: ${dadosMapeados.CandidatoAprovado || 'N/A'}\nVerifique os campos antes de salvar.`, "Importação Concluída");
        } catch (error) {
    ...
    let horaSai = "";
    let diasSemana = "";
    const turnoTexto = r["TURNO (INSERIR HORÁRIO, ESCALA, DSR)"] || r["TURNO"] || "";
    const timeMatch = turnoTexto.match(/(\d{2}:\d{2})/g);
    if (timeMatch && timeMatch.length >= 1) horaEnt = timeMatch[0];
    if (timeMatch && timeMatch.length >= 2) horaSai = timeMatch[1];
    if (turnoTexto.includes("6x1") || turnoTexto.includes("6X1")) diasSemana = "SEG A SAB";
...
function openVagasModal(vagaData = null, isCopy = false) {
    ...
    reader.onload = (e) => {
        try {
            ...
            if (vagaData.rg) f.rg_candidato.value = vagaData.rg;
...
async function fetchVagas() {
    ...
    const cargos = [...new Set(vagasData.map(v => v.Cargo).filter(Boolean))].sort();
    ...
    const fCargo = filterCargo ? filterCargo.value : '';
