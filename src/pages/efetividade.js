...
async function fetchData(startDate, endDate, turno) {
    ...
    const dsrLogQuery = supabase
        ...
        .lt('DataAlteracao', endISONextDay)
    ...
    const [colaboradores, preenchimentos, ferias, dsrLogs, afastamentos] = await Promise.all([
...
function processEfetividade(
    ...
    for (const registro of afastamentos) {
        if (registro.NOME && registro['DATA INICIO'] && registro['DATA RETORNO']) {
            ...
            const dataFimAfastamento = _ymdLocal(dataRetornoObj);
            if (dataFimAfastamento >= dataInicio) {
    ...
    for (const key of groupKeys) {
        ...
        for (const date of dates) {
            ...
            const elegiveis = colaboradoresDoGrupo.reduce((acc, c) => {
                ...
                if (nomesAfastados.has(nomeColaborador)) return acc;
                ...
                if (!isDSR) acc.push({...c, DSR_do_dia: historicalDSR});
                return acc;
            }, []);
...
async function generateReport() {
    ...
    try {
        ...
        const {colaboradores, preenchimentos, ferias, dsrLogs, afastamentos} = await fetchData(
            startDate,
            endDate,
            state.turnoAtual
        );
        ...
        if (groupKeys.length > 0) {
            groupKeys.sort((keyA, keyB) => {
                ...
                const pendenteCountA = statusesA.filter((s) => s === 'PEN').length;
