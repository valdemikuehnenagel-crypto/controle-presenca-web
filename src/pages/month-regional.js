function simplifyKpiName(kpiName) {
    ...
    const isPercentage = percentageKPIs.some(pkpi => simplifiedKpi.includes(pkpi));    if (isPercentage) {
    ...
    const numberValue = Number(value);
    const simplifiedKpi = simplifyKpiName(kpiName).toLowerCase();    switch (simplifiedKpi) {
        case '%cia':
        case '%coa':
...
let DATA_MODEL = null;function formatMonthLabel(ym) {
    ...
    const macroOptions = (macros || []).sort().map(m => `<option value="${m}">${m}</option>`).join('');    const allCodes = new Set();
    (kpis || []).forEach(kpi => {
    ...
    if (codigoToMacro) Object.keys(codigoToMacro).forEach(c => allCodes.add(c));    const serviceOptions = Array.from(allCodes).sort().map(c => `<option value="${c}">${c}</option>`).join('');    filtersEl.innerHTML = `
  ...
  `;
    headerEl.style.display = 'flex';    document.getElementById('mr-filter-macro').addEventListener('change', (e) => {
    ...
    if (!DATA_MODEL) return;    const {
        ...
        macros,
        codigoToMacro,
    } = DATA_MODEL;    let monthPool = months.slice();
    if (filterState.month) {
        monthPool = monthPool.filter(m => m === filterState.month);
    }
    const filteredMonths = monthPool;    const masterOrder = [
    ...
    if (codigoToMacro) Object.keys(codigoToMacro).forEach(c => allCodesMaster.add(c));    let visibleCodes = Array.from(allCodesMaster).sort();
    ...
    if (!container) return;
    let tableHTML = '';    if (kpisToShow.length > 0 && visibleCodes.length > 0 && filteredMonths.length > 0) {
        const totalColumns = 2 + (filteredMonths.length * 2);        tableHTML = `
    ...
    `;        tableHTML += '<thead><tr class="header-row-1">';
        ...
        kpisToShow.forEach((kpi, kpiIndex) => {
            ...
            const simplifiedKpi = simplifyKpiName(kpi);            codesForThisKpi.forEach((codigo, codigoIndex) => {
                tableHTML += '<tr>';
                if (codigoIndex === 0) {
                    tableHTML += `<td class="sticky-kpi" rowspan="${codesForThisKpi.length}">${simplifiedKpi}</td>`;
                }
                tableHTML += `<td class="sticky-codigo">${codigo}</td>`;                filteredMonths.forEach((currentMonthKey) => {
                    const currentMonthIndexInAll = DATA_MODEL.months.findIndex(m => m === currentMonthKey);
                    const previousMonthKey = (currentMonthIndexInAll + 1 < DATA_MODEL.months.length) ? DATA_MODEL.months[currentMonthIndexInAll + 1] : null;                    const currentData = data[currentMonthKey]?.[kpi]?.[codigo];
                    const previousData = previousMonthKey ? data[previousMonthKey]?.[kpi]?.[codigo] : null;                    const currentResult = currentData?.resultado ?? null;
                    const previousResult = previousData?.resultado ?? null;                    let deltaMom = null;
                    ...
                    const resultadoClass = getResultadoClass(kpi, currentResult);                    tableHTML += `<td class="res ${resultadoClass}">${resultadoFormatado}</td>`;
    ...
    if (!container) return;    if (loadingIndicator) loadingIndicator.style.display = 'flex';
    container.innerHTML = '';    try {
        ...
        } = result;        if (!months?.length || !kpis?.length) {
    ...
    DATA_MODEL = null;    filterState.macro = '';
    filterState.service = '';
    filterState.gerente = '';
    filterState.month = '';
}